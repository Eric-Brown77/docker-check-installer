...
if [ -t 0 ]; then
    ...
    # 通过管道运行
    INTERACTIVE=false
    echo "检测到脚本通过管道运行，无法接收直接的交互式输入。"
    echo "请从以下选项中输入数字选择安装方式："
fi
...
if [ "$INTERACTIVE" = true ]; then
    ...
    echo "请复制以下命令并粘贴到终端执行，其中X替换为您的选择 [1-4]："
    echo "echo X | bash"
    read choice
fi
...
if command -v docker &> /dev/null; then
    ...
    if [ "$INTERACTIVE" = true ]; then
        ...
        echo "提示：如需将用户添加到docker组（免sudo运行），请执行："
